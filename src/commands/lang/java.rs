...
For example if you write `!java 2 + 2` it'll actually execute:
```java
...
```"#]
pub async fn java(ctx: &Context, msg: &Message, args: Args) -> CommandResult {
...
",
    ...
    let res = execute_java(&code).await?;
    let res = format!("```\n{}\n```", res);
...
pub async fn execute_java(code: &str) -> Result<String> {
    let request = json!({
      "code": code,
      "language": "java",
      "input": "",
