...
For example if you write `!node 2 + 2` it'll actually execute:
```node
console.log(2 + 2)
```"#]
pub async fn node(ctx: &Context, msg: &Message, args: Args) -> CommandResult {
    let typing = Typing::start(ctx.http.clone(), *msg.channel_id.as_u64())?;

    let code = format!("console.log({})", args.rest());

    let res = execute_node(&code).await?;
    let res = format!("```\n{}\n```", res);
...
pub async fn execute_node(code: &str) -> Result<String> {
    let request = json!({
      "code": code,
      "language": "node",
      "input": "",
