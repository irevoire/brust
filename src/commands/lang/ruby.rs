...
For example if you write `!ruby 2 + 2` it'll actually execute:
```ruby
p 2 + 2
```"#]
pub async fn ruby(ctx: &Context, msg: &Message, args: Args) -> CommandResult {
    let typing = Typing::start(ctx.http.clone(), *msg.channel_id.as_u64())?;

    let code = format!("p {}", args.rest());

    let res = execute_ruby(&code).await?;
    let res = format!("```\n{}\n```", res);
...
pub async fn execute_ruby(code: &str) -> Result<String> {
    let request = json!({
      "code": code,
      "language": "rb",
      "input": "",
